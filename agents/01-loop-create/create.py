"""
Agente gerador de codigo Java do zero.

Uso:
  python3 create.py "Crie um CRUD completo para RepairShop com id UUID, name, countryCode e status"
  python3 create.py "Crie a entidade Product com name, category e price" --guideline guideline.md
"""
...
TOOLS = [
    {
        ...
        "function": {
            ...
            "description": (
                "Cria um arquivo Java no diretorio de output. "
                "Use um arquivo por classe. Chame para cada classe: entity, record, enum, "
                "repository, specifications, service interface, service impl, controller."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    ...
                    "content": {
                        "type": "string",
                        "description": "Conteudo completo do arquivo Java"
                    }
                },
    ...
    },
    {
        ...
        "function": {
            "name": "finalizar",
            "description": "Indica que todos os arquivos foram gerados. Chame apenas quando tiver criado tudo.",
            "parameters": {
                "type": "object",
                "properties": {
                    "message": {
                        "type": "string",
                        "description": "Resumo do que foi gerado"
                    }
                },
                "required": ["message"]
            }
...
def execute_tool(name: str, args: dict, created_files: list) -> str:
    if name == "escrever_arquivo":
        ...
        return f"Arquivo '{path}' criado ({len(content)} bytes)."
    if name == "finalizar":
        return "FINALIZADO"
...
def run(user_request: str, guideline_path: str | None):
    guideline = ""
    if guideline_path:
    ...
    if guideline:
        system_prompt = (
            ...
            "Gere codigo de producao seguindo rigorosamente o guideline abaixo.\n\n"
            "Regras obrigatorias:\n"
            ...
            "Crie os arquivos um de cada vez usando escrever_arquivo.\n"
            "Ao terminar tudo, chame finalizar com um resumo.\n\n"
            f"--- GUIDELINE COMPLETO ---\n{guideline}"
        )
    else:
        system_prompt = (
            "Voce e um desenvolvedor Java senior especialista em Spring Boot 3.x.\n"
            "Gere codigo de producao limpo e bem estruturado.\n\n"
            "Crie os arquivos um de cada vez usando escrever_arquivo.\n"
            "Ao terminar tudo, chame finalizar com um resumo."
        )

    print(f"\n{BOLD}{'='*60}{RESET}")
    print(f"{BOLD}  CREATE — GERADOR DE CODIGO JAVA{RESET}")
    ...
    print(f"  {DIM}Streaming SSE. O modelo gera arquivo por arquivo.{RESET}\n")
    ...
    while True:
        ...
        spinner = Spinner("Gerando codigo")
        spinner.start()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Gera codigo Java do zero com ou sem guideline.")
    parser.add_argument("request", nargs="+", help="Descricao do que criar")
    parser.add_argument("--guideline", metavar="PATH", default=None,
                        help="Caminho para o arquivo de guideline (opcional)")
    args = parser.parse_args()

    run(" ".join(args.request), args.guideline)
